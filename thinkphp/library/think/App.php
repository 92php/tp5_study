...
class App extends Container
{
    ...
    public function initialize()
    {
        ...
        // 加载环境变量配置文件 （默认框架是没有.env文件）
        if (is_file($this->rootPath . '.env')) {
            //思考？为什么$this->env能定位到 load 方法
            // App类和Container类 都没有load方法  最后触发的方法是：Container类中 __get
        ...
        // 注册异常处理类（这个地方代码不友好，其实init方法里面的containerConfigUpdate 这个方法有做异常处理，有点冗余）
    ...
    public function init($module = '')
    {
        ...
        $path   = $this->appPath . $module;//path有两种，一种是application下，另一种是模块下 如index下
    ...
    public function run()
    {
        try {
            ...
            if (empty($dispatch)) {
                // 路由检测
                $dispatch = $this->routeCheck()->init(); //这个里面会再次调用 app类的 init方法
                //var_dump($dispatch); //think\route\dispath\Module
            }
    ...
    public function controller($name, $layer = 'controller', $appendSuffix = false, $empty = '')
    {
        ...
        //dump($this->parseClass($module, $layer, $empty, $appendSuffix));
        if (class_exists($class)) {
