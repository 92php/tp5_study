...
class RuleItem extends Rule
{
    ...
    public function setRule($rule)
    {
        ...
        if ('$' == substr($rule, -1, 1)) {
            // 是否完整匹配
            $rule = substr($rule, 0, -1); //halt($rule);
        ...
        if (false !== strpos($rule, ':')) {
            ...
            //halt($this->rule);  // hello/<name>      hello/<name? >
        } else {
            $this->rule = $rule;
        }
    ...
    protected function setRuleName($first = false)
    {
        ...
        if ($this->name) {
            //$this->rule 就是路由规则
            $vars = $this->parseVar($this->rule); // 返回 name=> 1 动态参数  2为可选参数
            $name = strtolower($this->name);

            if (isset($this->option['ext'])) {
                $suffix = $this->option['ext'];
            } elseif ($this->parent->getOption('ext')) {
                //dump($this->parent); //think\route\Domain => 父类rulegroup => 父类 rule
                //getOption方法在 rule类中
