...
class Route
{
    ...
    {
        $this->app     = $app;
        $this->request = $app['request']; //对象，用数组的方式使用 (App类继承了Container类，而Container类继承了ArrayAccess)
        $this->config  = $config;
    ...
    {
        //思考：为什么$this->group 能找到 RuleGroup类？？？
        //因为 Route类中的 __construct 方法中 调用了setDefaultDomain方法，而 setDefaultDomain方法中 使用了 Domain类，而Domain类继承了RuleGroup类
    ...
    public function check($url, $must = false)
    {
        ...
        //思考request对象时怎么找到的？？？ (container 中 offsetSet)
        // 默认路由解析
